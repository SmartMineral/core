...
contract('BitcoineumTest', function(accounts) {
    ...
    it("should have 10000000 divisible units", async function() {
        let token = await SmartMineral.new();
    ...
    it("should have a maximum supply of 21000000 SmartMineral", async function() {
        let token = await SmartMineral.new();
        let maximumSupply = await token.maximumSupply();
        assert.equal(maximumSupply.valueOf(), 21000000 * (10 ** 8), "SmartMineral supply should be 21000000 with 8 decimal places");
    });
    ...
    it("should correctly calculate the base mining reward", async function() {
        ...
        assert.equal(res.valueOf(), 100 * (10 ** 8), "At internal block 0, we reward 50 SmartMineral");
        res = await token.calculate_base_mining_reward(1)
        assert.equal(res.valueOf(), 100 * (10 ** 8), "At internal block 0, we reward 50 SmartMineral");
        res = await token.calculate_base_mining_reward(210000)
        assert.equal(res.valueOf(), 100 * (10 ** 8), "100 SmartMineral up until block 210000");
        res = await token.calculate_base_mining_reward(210001)
        assert.equal(res.valueOf(), 50 * (10 ** 8), "25 SmartMineral immediately after block 210000");
        res = await token.calculate_base_mining_reward(420001);
        assert.equal(res.valueOf(), 25 * (10 ** 8), "12.5 SmartMineral immediately after block 410000");
    });
    ...
    // Let's use the mock SmartMineral to test main functions


    it('should mock SmartMineral correctly', async function() {
        let token = await SmartMineralMock.new();
